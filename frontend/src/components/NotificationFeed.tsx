...
import { Bell, BellOff, CalendarCheck, CreditCard, UtensilsCrossed } from "lucide-react";
...
function typeIcon(type: string) {
  ...
  if (type === "ORDER_UPDATE" || type === "NEW_ORDER") return UtensilsCrossed;
  if (type === "DEPOSIT_RECEIPT" || type === "PAYMENT_REJECTED" || type === "PAYMENT_SUBMITTED" || type === "PAYMENT_PROOF_UPLOADED") {
    return CreditCard;
  }
  return Bell;
}

// Shared by the customer and staff/admin notifications pages — both hit the
// same GET /notifications/mine (scoped server-side by role, see
// NotificationController::mine), just inside a different sidebar shell.
export default function NotificationFeed({ subtitle }: { subtitle: string }) {
  ...
  return (
    <>
      ...
      <p className="mt-2 text-sm text-grill-brown/50">{subtitle}</p>
